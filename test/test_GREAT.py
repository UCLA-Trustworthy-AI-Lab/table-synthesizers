...
@pytest.mark.skipif(not GREAT_AVAILABLE, reason="GREAT not available due to missing dependencies")
...
@pytest.mark.skipif(not GREAT_AVAILABLE, reason="GREAT not available due to missing dependencies")
def test_GREAT_dataframe_support():
    ...
    print("GREAT DataFrame test passed successfully!")


@pytest.mark.skipif(not GREAT_AVAILABLE, reason="GREAT not available due to missing dependencies")
def test_GREAT_different_data_types():
    """Test GREAT with different data types"""
    np.random.seed(42)
    df = pd.DataFrame({
    ...
    print("GREAT different data types test passed!")


def test_GREAT_availability():
    """Test if GREAT is properly detected as available or not"""
    if GREAT_AVAILABLE:
        print("GREAT is available and registered in DEFAULT_MODELS")
    else:
        print("GREAT is not available due to missing synthcity dependencies (expected)")
        pytest.skip("GREAT not available due to missing dependencies")


if __name__ == "__main__":
    test_GREAT_availability()
    if GREAT_AVAILABLE:
        test_GREAT_initialization()
        test_GREAT_dataframe_support() 
        test_GREAT_different_data_types()