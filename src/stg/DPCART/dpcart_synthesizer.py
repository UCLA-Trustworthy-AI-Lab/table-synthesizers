...
from .dpcart import cart_synthesizer_dp_pruned


class DPCARTSynthesizer(BaseSynthesizer):
    """
    Differentially Private CART-based synthesizer for tabular data generation.
    
    This synthesizer uses DP-pruned Classification and Regression Trees to learn
    conditional dependencies between columns while preserving privacy.
    Only supports DataFrame input (not DataLoader).
    """
    
    def __init__(self, data_info=None, max_depth=None, epsilon_per_tree=1.0, random_state=None, **kwargs):
        super().__init__(data_info=data_info, **kwargs)
        self.max_depth = max_depth
        self.epsilon_per_tree = epsilon_per_tree
    ...
    def train(self, train_data, batch_size=32):
        ...
        if not isinstance(train_data, pd.DataFrame):
            raise ValueError("DPCARTSynthesizer only supports DataFrame input, not DataLoader")
        ...
        print(f"DPCART: stored {len(self.trained_data)} training samples")
        print(f"Numeric columns: {self.numeric_cols}")
        print(f"Categorical columns: {self.categorical_cols}")
        print(f"Privacy budget per tree: {self.epsilon_per_tree}")
        
        self.stop_threading()
        
    def _encode_categorical_data(self, df):
        """Encode categorical columns to integers for DPCART training."""
        encoded_df = df.copy()
    ...
    def _generate(self, n_samples):
        """Generate synthetic samples using DP-CART (decoded version)."""
    ...
    def _generate_encoded(self, n_samples):
        ...
        # Use the DP-CART synthesizer function on encoded data
        synthetic_df = cart_synthesizer_dp_pruned(
