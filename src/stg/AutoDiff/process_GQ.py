...
def convert_to_table(org_df, gen_output, threshold):
    ...
    cate_column_pairs = [(col.replace('Cate_', ''), col) for col in syn_df.columns if isinstance(col, str) and col.startswith('Cate_')]
    ...
    # Detect pairs of columns with column names 'Cate_X' and 'X'
    bin_column_pairs = [(col.replace('Binary_', ''), col) for col in syn_df.columns if isinstance(col, str) and col.startswith('Binary_')]
